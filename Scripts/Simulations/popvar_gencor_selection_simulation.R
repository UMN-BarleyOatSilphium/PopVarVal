...
simulation_out <- mclapply(X = param_df_split, FUN = function(core_df) {
  ...
  for (i in seq_along(results_out)) {
    ...
    weights <- c(0.5, 0.5) / sqrt(diag(var(tp1$pheno_val$pheno_mean[,-1])))
    ...
    pred_out_select <- pred_out_select[order(pred_out_select$index, decreasing = TRUE)[1:cross_select],]
    cb_select_muspC <- select(pred_out_select, contains("parent"))
    ...
    pred_out_select_mean <- pred_out_select_mean[order(pred_out_select_mean$mu_index, decreasing = T)[1:cross_select],]
    ...
    pred_out_select_musp <- pred_out %>% 
      ...
      mutate(mu_index = (trait1 * weights[1]) + (trait2 * weights[2]))
    pred_out_select_musp <- pred_out_select_musp[order(pred_out_select_musp$mu_index, decreasing = TRUE)[1:cross_select],]
    ...
    ## Combine crossing blocks
    cb_list <- list(muspC = cb_select_muspC, mean = cb_select_mean, musp = cb_select_musp, rand = cb_select_rand)
    ...
    selections <- cycle1_list %>%
      map(~select_pop(pop = ., intensity = 0.1, index = weights, type = "genomic")) %>%
      map("geno_val")
    ...
    ## Summarize the mean and cor of each/both traits
    response <- selections %>%
      mutate(muspC = map(summ, "muspC"), musp = map(summ, "musp"), mean = map(summ, "mean"), rand = map(summ, "rand")) %>%
      select(-summ) %>% 
